...
class KNN:
    ...
    def buildModel(cls, train_x, test_x, train_y, test_y, params = {"n_neighbors" : 5, "weights" : "distance", "algorithm" : "auto"}):
        ...
        def metrics(y_preds, y_true):
            ...
            FPR = fp/(fp+tn)
            FNR = fn/(fn+tp)
            return round(accuracy, 3), round(precision, 3), round(recall, 3), round(f1, 3), round(FPR, 3), round(FNR, 3)
        
        accepted_params = ["n_neighbors", "weights", "algorithm"]
        passed_params = [params.get(param) for param in accepted_params]
        if len(passed_params) == len(accepted_params):
            clf = KNeighborsClassifier(n_neighbors = params.get("n_neighbors"), weights = params.get("weights"), algorithm= params.get("algorithm"))
        ...
        clf = KNeighborsClassifier() # no params specified
        clf.fit(X_train, y_train)
        y_preds = clf.predict(X_test)
        #return the metrics
